...
from opencd_custom.models.decode_heads.diff_vit import DiffTransformerBlock
    ...
    def encode_feature(self, x: torch.Tensor,
        ...
        if 'MixVisionTransformer' in self.encoder_type:
            ...
            for index, layer_name in enumerate(self.side_encoder.res_layers, start=0):
                ...
                if index in self.fusion_index:
                    if cls_token:
                        if clip_features[fused_index][0].dim() == 3:
                            B, hw_shape, C = clip_features[fused_index][0].shape
                            hw = int(math.sqrt(hw_shape))
                            clip_feature = clip_features[fused_index][0].reshape(B, hw, hw, C).permute(0, 3, 1, 2).contiguous()
                        else:
                            clip_feature = clip_features[fused_index][0]
                        x = self.fuse_clip(fused_index, x, clip_feature)
                    else:
                        if clip_features[fused_index].dim() == 3:
                            B, hw_shape, C = clip_features[fused_index].shape
                            hw = int(math.sqrt(hw_shape))
                            clip_feature = clip_features[fused_index].reshape(B, hw, hw, C).permute(0, 3, 1, 2).contiguous()
                        else:
...
class BitemporalAdapterHead(BaseDecodeHead):
    def __init__(self,
        ...
        #self.diffT = DiffTransformerBlock()
